...
def compute_metrics(s_perm_mat, P1_gt, P2_gt, R_gt, T_gt, device, usepgm=True, userefine=False):
    # compute r,t
    R_pre, T_pre = compute_transform(s_perm_mat, P1_gt, P2_gt, R_gt, T_gt, usepgm=usepgm, userefine=userefine)
    ...
    clip_val = torch.Tensor([0.1]).to(device)
    P1_transformed = torch.from_numpy(transform(torch.cat((R_pre, T_pre[:, :, None]), dim=2).detach().cpu().numpy(),
                                                P1_gt.detach().cpu().numpy())).to(P1_gt)
...
def compute_othermethod_metrics(transform_para, P1_gt, P2_gt, R_gt, T_gt, device, usepgm=True, userefine=False):
    # compute r,t
